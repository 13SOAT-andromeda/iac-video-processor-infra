...
**Data:** 2026-07-11 (atualizado 2026-07-13 — EKS volta ao escopo)
...
- **AWS Load Balancer Controller** (Helm, instalado via pipeline) — provisiona dinamicamente o Application Load Balancer compartilhado a partir dos recursos `Ingress` de cada serviço no cluster (ver seção 6).
...
- **Tag de cluster EKS:** `video-processor-eks` — usada pelo `iac-video-processor-gateway` para achar o security group do cluster (`data.aws_security_group` filtrando `tag:aws:eks:cluster-name`) e o ALB compartilhado (`data.aws_lb` filtrando `kubernetes.io/cluster/video-processor-eks = owned`, tag aplicada automaticamente pelo AWS Load Balancer Controller).
...
Cada serviço (`users-api` e futuros) declara seu próprio `Ingress` com `alb.ingress.kubernetes.io/group.name: video-processor-shared` (mesmo grupo) para que o controller funda todos num único ALB — ver `video-processor-users-api`, seção 8.2, e `iac-video-processor-gateway`, seção 7.
...
- **`video-processor-users-api`** (agora EKS): **não** instancia mais o módulo Lambda. Em vez disso, tem `k8s/base/` + `k8s/overlays/aws/` (Deployment, Service, Ingress, HPA — Kustomize, não Helm) aplicados via `kubectl apply -k` no próprio pipeline de deploy do serviço, contra o cluster provisionado aqui. Constrói e publica imagem no ECR deste repo. Ver `video-processor-users-api`, seção 8.2, para o detalhe completo.
